      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-1">
            {navItems.map((item) => (
              <a
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 relative group ${
                  scrolled ? 'text-gray-600 hover:text-emerald-600' : 'text-white hover:text-emerald-400'
            ...
            <button href="#donate" className={`px-6 py-2 rounded-full text-sm font-semibold transition-all duration-300 ${
              scrolled 
          ...
          {navItems.map((item) => (
            <a
              ...
              className={`block px-4 py-2 rounded-lg text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 transition-colors duration-300 ${
                activeSection === item.id ? 'bg-emerald-50 text-emerald-600 font-semibold' : ''
              }`}
            >
              {item.label}
            </a>
          ))}
          <button href="#donate" className="w-full mt-4 px-6 py-3 bg-emerald-600 text-white rounded-lg font-semibold hover:bg-emerald-700 transition-colors duration-300">
        Volunteer
          </button>
        </div>
      </div>
